...
async function translateWithAI(data, targetLang, context = "") {
  // Extract only the translatable strings from the object
  const { strings, skeleton } = extractStrings(data);

  if (Object.keys(strings).length === 0) return data; // nothing to translate
